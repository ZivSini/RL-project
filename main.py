...
def episode(env, policy, render, r_max=0, t_max=0):
...
def experiment(env, policy, n_episodes, r_max=0, t_max=0):
    ...
    for i in range(n_episodes):
        val = episode(env, policy, False, r_max, t_max)
        # print('episode:{}, reward {}'.format(i,val))
        rewards[i] = val
        # let the pc cool down after running 1 episode
        time.sleep(120)
...
def render_episode(env, policy, render=True, r_max=0, t_max=0):
    # create the empty list to contain game memory
    # memory = deque(maxlen=1000)
...
memory_file = open('memory_deque.obj', 'rb')
memory = pickle.load(memory_file)

# experiment(env, policy_q_nn, n_episodes)

model.save_weights('./checkpoints/my_checkpoint')
memory_file = open('memory_deque.obj', 'wb')
pickle.dump(memory, memory_file)

render_episode(env, policy_q_nn, True, 0, 0)
